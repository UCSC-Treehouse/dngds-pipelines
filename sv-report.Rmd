...
vcf.2mrare = subset(vcf.sn, METHODS==2 & AFMAX<.01)
d.df = distanceToNearest(vcf.2mrare, genes.gr) %>% as.data.frame
info(vcf.2mrare)$SELGENEDIST = NA
info(vcf.2mrare)$SELGENEDIST[d.df$queryHits] = d.df$distance
info(vcf.2mrare)$SELGENE = NA
info(vcf.2mrare)$SELGENE[d.df$queryHits] = genes.gr$gene[d.df$subjectHits]

vcf.2mrare.sel = subset(vcf.2mrare, SELGENEDIST<1e6)
vcf.2mrare.sel = vcf.2mrare.sel[order(-abs(info(vcf.2mrare.sel)$SVLEN))]

t = lapply(unique(info(vcf.2mrare.sel)$SELGENE), function(gene.name){
  vcf = subset(vcf.2mrare.sel, SELGENE==gene.name)
  vars = tibble(
...
vcf.2mrare.large = subset(vcf.2mrare, abs(SVLEN)>1e3)
vcf.2mrare.large = vcf.2mrare.large[order(-abs(info(vcf.2mrare.large)$SVLEN))]

vars = tibble(
  pos=linkPos(as.character(seqnames(rowRanges(vcf.2mrare.large))), start(rowRanges(vcf.2mrare.large)), info(vcf.2mrare.large)$SVTYPE, info(vcf.2mrare.large)$SVLEN, format=out.format),
  type=info(vcf.2mrare.large)$SVTYPE,
  size=info(vcf.2mrare.large)$SVLEN,
  freq=info(vcf.2mrare.large)$AFMAX,
  simp.rep=info(vcf.2mrare.large)$ol.simple.repeat)
if(info(vcf.2mrare.large)$METHOD[1]=='Sniffles'){
  vars$reads=info(vcf.2mrare.large)$RE
} else {
  vars$reads=info(vcf.2mrare.large)$SUPPORT
}
if(out.format == 'pdf'){
  vars %>% kable(booktabs=TRUE, escape=FALSE) %>%
    cat
