...
if(!is.null(t.df)){
  ...
  out.tsv = rbind(out.tsv,
                  ...
                  select(chr, start, end, type, size, gene_list, gene, pli, gt, freq, dgv, clingen,
                         simp.rep, effect))
}
```
...
if(!is.null(t.df)){
  ...
  out.tsv = rbind(out.tsv,
                  ...
                  mutate(start=pos, end=ifelse(type=='INS', pos, pos+size), gene_list='not-in-list') %>% 
                  select(chr, start, end, type, size, gene_list, gene, pli, gt, freq, dgv, clingen,
                         simp.rep, effect))
}
```
...
```{r writetsv}
out.tsv = unique(out.tsv)
outgz = gzfile('svs-in-gene-list.tsv.gz', 'w')
write.table(out.tsv, file=outgz, sep='\t', row.names=FALSE, quote=FALSE)
close(outgz)
```
