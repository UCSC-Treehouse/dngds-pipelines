...
if(!is.null(t.df)){
  ...
  t.df %>% filter(freq<.01, !hsvlr) %>% formatGeneTable %>%
    select(gene, pli, type, size, pos, gt, freq, dgv, clingen, cons, simp.rep, effect) %>% 
    datatable(filter='top', escape=FALSE, options=list(pageLength=50))
}
```
...
if(!is.null(t.df)){
  ...
  t.df %>% filter(comhet) %>% merge(genes.list) %>% formatGeneTable %>%
    select(gene_list, gene, pli, type, size, pos, gt, freq, dgv, clingen, cons, simp.rep, effect) %>% 
    datatable(filter='top', escape=FALSE, options=list(pageLength=50))
}
```
